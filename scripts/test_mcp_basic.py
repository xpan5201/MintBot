...
async def test_mcp_time_server():
    """测试 MCP 时间服务器"""
    print("正在连接 MCP 时间服务器...")

    # 创建服务器参数
    server_params = StdioServerParameters(
        command=r"D:\Users\xpan5201\anaconda3\envs\mintchat\python.exe",
        ...
        async with stdio_client(server_params) as (read, write):
            ...
            async with ClientSession(read, write) as session:
                ...
                if tools.tools:
                    ...
                    print(f"\n正在调用工具: {tool_name}")
                    result = await session.call_tool(tool_name, arguments={"timezone": "Asia/Shanghai"})
...
if __name__ == "__main__":
    success = asyncio.run(test_mcp_time_server())
