...
class AsyncDatabasePool:
    ...
    def __init__(
        ...
        if not HAS_AIOSQLITE:
            raise RuntimeError("aiosqlite 未安装，请先运行: `uv pip install aiosqlite`")
    ...
    async def _create_connection(self) -> aiosqlite.Connection:
        ...
        try:
            ...
            logger.debug(f"创建新的异步数据库连接 (总数: {total_connections})")
            return conn
    ...
    async def close_all(self):
        ...
        async with self._lock:
            ...
            while True:
                try:
                    idle_connections.append(self._available.get_nowait())
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        # 创建表
        await pool.execute(
            "CREATE TABLE IF NOT EXISTS test (id INTEGER PRIMARY KEY, name TEXT)", commit=True
        )

        # 插入数据
        await pool.execute("INSERT INTO test (name) VALUES (?)", ("Alice",), commit=True)

        # 批量插入
        await pool.execute_many(
            "INSERT INTO test (name) VALUES (?)", [("Bob",), ("Charlie",), ("David",)], commit=True
        )

        # 查询数据
        results = await pool.execute("SELECT * FROM test", fetch_all=True)
