...
class MemoryPersistenceConfig(BaseModel):
    ...
    enable_delayed_persist: bool = Field(
        default=False,
        description="启用延迟持久化可以显著提升性能，但可能在异常退出时丢失少量数据"
    )
    ...
    persist_interval: float = Field(
        ...
        ge=5.0,
        le=300.0,
        description="定时持久化间隔，单位秒"
    )

    # 是否在程序退出时强制持久化
    force_persist_on_exit: bool = Field(
        default=True,
        description="程序退出时强制持久化所有缓冲数据"
    )
...
class DatabaseConfig(BaseModel):
    ...
    enable_connection_pool: bool = Field(
        default=False,
        description="启用连接池可以提升20-30%性能，但需要重构代码使用上下文管理器"
    )
...
class CacheConfig(BaseModel):
    ...
    cache_ttl: Optional[float] = Field(
        ...
        ge=60.0,
        le=3600.0,
        description="缓存过期时间，None表示永不过期"
    )

    # 是否启用嵌入向量缓存
    enable_embedding_cache: bool = Field(
        default=True,
        description="缓存嵌入向量可以避免重复计算"
    )
...
class AsyncConfig(BaseModel):
    ...
    enable_async_memory_search: bool = Field(
        default=False,
        description="异步记忆检索可以减少首字延迟，但需要重构代码"
    )
    ...
    thread_pool_size: int = Field(
        ...
        ge=1,
        le=16,
        description="异步任务线程池大小"
    )

    # 是否启用记忆预加载
    enable_memory_preload: bool = Field(
        default=False,
        description="预加载常用记忆可以提升响应速度"
    )
...
class PerformanceConfig(BaseModel):
    ...
    memory_persistence: MemoryPersistenceConfig = Field(
        default_factory=MemoryPersistenceConfig
    )
