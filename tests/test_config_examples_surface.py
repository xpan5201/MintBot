...
def test_dev_config_covers_required_surface() -> None:
    data = _load_yaml_mapping(Path("config.dev.yaml"))
    missing = [p for p in DEV_CONFIG_REQUIRED_PATHS if not has_config_path(data, p)]
    assert not missing, f"missing keys: {missing}"
