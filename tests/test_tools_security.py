...
def test_file_tools_block_sensitive_files() -> None:
    registry = ToolRegistry()
    try:
        for filename in ("config.user.yaml", "config.dev.yaml", "config.yaml"):
            result = registry.execute_tool("read_file", filepath=filename, base_dir=".")
            assert "安全" in result
    finally:
        registry.close()
...
def test_file_tools_write_read_and_list_under_tmp(tmp_path: Path) -> None:
    ...
    try:
        write_result = registry.execute_tool(
            "write_file", filepath="a.txt", content="hello", base_dir=base_dir
        )
        ...
        (tmp_path / "config.user.yaml").write_text("secret", encoding="utf-8")
        (tmp_path / "config.dev.yaml").write_text("secret", encoding="utf-8")
        ...
        assert "config.user.yaml" not in list_result
        assert "config.dev.yaml" not in list_result
