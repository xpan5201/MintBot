...
- `log_quiet_libs`：需降噪的第三方 logger 列表，默认 `["httpx","asyncio","urllib3","charset_normalizer","multipart.multipart"]`。
- `log_quiet_level`：降噪级别，默认 `WARNING`。
- `log_drop_keywords`：包含任意关键词的日志将被直接丢弃（前后端共用），默认过滤 HTTP 低级调试噪声。
...
如需调整，将配置写入 `config.yaml` 或设置对应环境变量，程序启动时会自动应用。***
