# MintChat 更新日志（Legacy）

> 说明：本文件为历史版本摘录，内容可能包含已在 Phase 7 移除/替换的组件（如 LangChain/LangGraph）。
> 当前迁移与验收请以 `archives/` 中对应计划的决策记录与 Gate checklist 为准；开发历史以 `git log` 为准。

本文档保留旧版本的“人工整理摘要”，用于回溯历史决策与演进脉络。
